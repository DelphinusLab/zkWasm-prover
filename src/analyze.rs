...
pub(crate) fn analyze_involved_advices<C: CurveAffine>(
    ...
    for lookup in &pk.vk.cs.lookups {
        if lookup.input_expressions.len() > 1 {
            ...
            collect_involved_advices(&lookup.input_expressions[..], &mut lookups_involved_units);
            collect_involved_advices(&lookup.table_expressions[..], &mut lookups_involved_units);
        }
    }
    ...
    for shuffle in &pk.vk.cs.shuffles.0 {
        collect_involved_advices(&shuffle.input_expressions[..], &mut shuffle_involved_units);
        collect_involved_advices(
